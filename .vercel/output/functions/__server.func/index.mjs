...
var grokOgIdentity = { "site": {
	"title": "Cozy AI Studio",
	"short_name": "Cozy Studio",
	"card": "custom",
	"color": "12110F",
	"description": "Brief to live preview. A calm studio for shipping product surfaces.",
	"image": "/og.jpg"
} };
//#endregion
...
function renderWebManifest(hostHeader, site) {
	const identity = site && typeof site === "object" ? site : readOgSite();
	const name = String(identity.title ?? "").trim() || appNameFromHost(hostHeader);
	const shortName = String(identity.short_name ?? "").trim() || name;
	const theme = themeColorFromSite(identity);
...
function themeColorFromSite(site = {}) {
	const raw = String(site.color ?? "").trim();
	const hex = raw.startsWith("#") ? raw : raw ? `#${raw}` : "";
	return /^#[0-9a-fA-F]{6}$/.test(hex) ? hex.toLowerCase() : "#12110f";
}
function isManifestPath(pathname) {
	const path = String(pathname ?? "").split("?")[0];
	return path === "/__grok/manifest.webmanifest" || path === "/__grok/manifest.json" || path === "/manifest.webmanifest" || path === "/manifest.json";
}
function manifestHttp(method, hostHeader, site) {
	const body = renderWebManifest(hostHeader, site);
	const head = String(method ?? "GET").toUpperCase() === "HEAD";
	/** @type {Record<string, string>} */
	const headers = {
		"content-type": "application/manifest+json; charset=utf-8",
		"cache-control": "no-cache"
	};
	if (!head) headers["content-length"] = String(Buffer.byteLength(body));
	return {
		status: 200,
		headers,
		body: head ? null : body
...
async function grokPwaMiddleware(event, next) {
	...
	if (isManifestPath(path) && (method === "GET" || method === "HEAD")) {
		const payload = manifestHttp(method, requestHost(event));
