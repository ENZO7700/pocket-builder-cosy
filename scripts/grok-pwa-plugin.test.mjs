...
test("renders the manifest with the site identity and install icons", () => {
  const manifest = JSON.parse(renderWebManifest("wild-race.grok.me"));
  assert.equal(manifest.name, "Cozy AI Studio");
  assert.equal(manifest.short_name, "Cozy Studio");
  assert.equal(manifest.display, "standalone");
  assert.equal(manifest.start_url, "/");
  assert.equal(manifest.scope, "/");
  assert.equal(manifest.theme_color, "#12110f");
  assert.equal(manifest.background_color, "#12110f");
  assert.ok(manifest.icons.some((icon) => icon.sizes === "192x192"));
  assert.ok(manifest.icons.some((icon) => icon.sizes === "512x512"));
  assert.ok(manifest.icons.some((icon) => icon.purpose === "maskable"));
});

test("falls back to the host slug when site has no title", () => {
  const manifest = JSON.parse(renderWebManifest("wild-race.grok.me", {}));
  assert.equal(manifest.name, "Wild Race");
  assert.equal(manifest.short_name, "Wild Race");
});

test("HEAD and GET share status and content-type", () => {
  const get = manifestHttp("GET", "localhost");
  const head = manifestHttp("HEAD", "localhost");
  assert.equal(get.status, 200);
  assert.equal(head.status, 200);
  assert.equal(get.headers["content-type"], "application/manifest+json; charset=utf-8");
  assert.equal(head.headers["content-type"], get.headers["content-type"]);
  assert.equal(head.body, null);
  assert.match(String(get.body), /Cozy AI Studio/);
});
